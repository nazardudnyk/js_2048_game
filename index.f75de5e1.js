...
function addTile() {
    ...
    for(let i = 0; i < magicValue; i++){
        for(let j = 0; j < magicValue; j++)if (state[i][j] !== newState[i][j]) {
            changed = true;
            break;
        }
        if (changed) break;
...
function move(e) {
    ...
    for(let i = 0; i < magicValue; i++){
        for(let j = 0; j < magicValue; j++)if (newState[i][j] !== state[i][j]) {
            state = newState;
            addTile();
            render();
            if (isWin) {
...
function transposeState() {
    newState = newState[0].map((_, colIndex)=>newState.map((row)=>row[colIndex]));
}
function up() {
    transposeState();
    left();
...
function checkRows() {
    for(let i = 0; i < magicValue; i++){
        if (newState[i].some((cell)=>cell === 0) || newState[i].some((cell, j)=>cell === newState[i][j + 1])) return true;
    }
    return false;
}
function checkColumns() {
    for(let i = 0; i < magicValue; i++){
        if (newState[i].some((cell, j)=>cell === newState[i][j + 1])) return true;
    }
    return false;
}
